...
public class FormHandler extends HttpServlet {
	...
	try {
		...
		userObject.setcPassword(request.getParameter("cPassword").trim());
		userObject.setEmail(request.getParameter("email").trim());
		userObject.setPhoneNumber(request.getParameter("pNumber").trim());
    ...
    private static void profileHandler(HttpServletRequest request, HttpServletResponse response, User userObject) throws SQLException, IOException {
	...
	if (result.next()) {
		...
		if (result.getString("name").equalsIgnoreCase(userObject.getName()) && result.getString("password").equals(userObject.getPassword())) {
	    ...
	    query = "INSERT INTO userAccount VALUES('" + userObject.getName() + "','" + userObject.getPassword() + "','" + userObject.getEmail()
		    + "')";
	...
	if (name.equals("") || password.equals("")) {
	    ...
	    if ((cPassword.equals("") || email.equals("") || phoneNumber.equals(""))) {
		errorMessage = "Form Field can't be empty!!";
		returnValue = false;
	    ...
	    } else if (email.indexOf("@") < 0 || email.length() < 4 || (email.lastIndexOf(".") - email.indexOf("@") < 1)
